...
volatile uint8_t applicationDebugKeyPassword[16U] __attribute__((section("_adkp_key")));
    ...
    /* ---------- Previously missing — required by hse_secure_boot.c ---------- */
    volatile uint8_t gRunSecureBootType = USE_BASIC_SECURE_BOOT;
    volatile uint8_t programmed_appdebugkey[16] = {0U};
    volatile uint8_t adkp_hash[16] = {0U};

    /* ---------- Moved from hse_host_format_key_catalogs.c ---------- */
    ...
    /* -----------------------------------------------------------------------
     * Global state variables shared across the library service modules.
     * Applications may override these with their own definitions (strong
     * symbols supersede these when the application provides them).
     * ---------------------------------------------------------------------- */

    /** @brief Global HSE service response – examined by service orchestration code. */
    volatile hseSrvResponse_t gsrvResponse = HSE_SRV_RSP_GENERAL_ERROR;

    /** @brief Attribute programming selector set by the application at run-time. */
    volatile eHSEFWAttributes gProgramAttributes = NO_ATTRIBUTE_PROGRAMMED;

    /** @brief IVT authentication enable flag set by the application at run-time. */
    volatile eHSEFWAttributes gEnableIVTAuthBit = NO_ATTRIBUTE_PROGRAMMED;

    /* gCryptoServicesExecuted is defined in hse_crypto.c — do not duplicate here. */

    /** @brief HSE FW version read back after getFwVersionService(). */
