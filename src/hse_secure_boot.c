...
/* Sign the application image using a device specific key */
    ...
    static hseSrvResponse_t VerifyKeys(uint8_t Index)
    {
        ...
        if (TRUE != IsTagLocationErased)
        {
            ...
            ASSERT(FLS_JOB_OK == HostFlash_Erase(HSE_HOST_BLOCK0_CODE_MEMORY, CMAC_TAG_CODE_FLASH_ADDRESS, 1U));
        }
        ...
        if (0U == Index) /* AES-CMAC */
        {
            ...
            write_status = HostFlash_Program(HSE_HOST_BLOCK0_CODE_MEMORY,
 ...
 ******************************************************************************/
        ...
        const uint32_t CodeLength)
    {
        ...
        if (ADDR_NOT_NULL(pImg))
        {
            ...
            {
                IsTagLocationErased = TRUE;
                ASSERT(FLS_JOB_OK == HostFlash_Erase(HSE_HOST_BLOCK0_CODE_MEMORY, (uint32_t)(pImg + APP_HEADER_LENGTH + CodeLength), BOOT_IMG_TAG_LEN));
            }
