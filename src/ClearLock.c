/*******************************************************************************
*                                                                              *
*  FILE NAME     : ClearLock.c                                                 *
*  FUNCTION      : ClearLock                                                   *
*  DESCRIPTION   : This function will clear the lock bit which corresponded to *
*                  that sector.                                                *
...
*                                                                              *
*  RETURN        : FLS_JOB_OK         - Clear the lock bit successfully        *
*                : FLS_JOB_FAILED     - Clear the lock bit unsuccessfully      *
*******************************************************************************/
/*==================================================================================================
*
*   Copyright 2022 NXP.
...
*
*/
#include "hse_fls_registers.h"
#include "hse_fls_api.h"
#include "hse_host_flash.h"

Fls_CheckStatusType ClearLock( uint32 Fls_VirtualSectors, uint8 u8DomainIdValue )
{
    Fls_CheckStatusType eReturnCode = FLS_JOB_FAILED;     /* return code */
    uint32 SectorId = 0U;
    uint8 u8LockDomainIDValue = 0U;
    uint32 u32TempLockMasterRegister;
    ...
    if ((uint32)Fls_VirtualSectors < VIRTUAL_CODE_FLASH_BLOCK_0_FIRST_SEC_ID)
    {
        ...
        /*
        * @violates @ref ClearLock_c_REF_2 A conversion should not be performed between a pointer to object and an integer type
        * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
        */
        if(0U != REG_BIT_GET32(PFLASH_DFLASHBLK_SETSLOCK_ADDR32, 1U << SectorId))
        {
            u32TempLockMasterRegister = PFLASH_DFLASHBLK_LOCKMASTER_S0_ADDR32 + ((SectorId/4U)*4U);
            /*
            * @violates @ref ClearLock_c_REF_2 A conversion should not be performed between a pointer to object and an integer type
            * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
            */
            ...
            else
            {
                ...
                * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
                */
                REG_BIT_CLEAR32(PFLASH_DFLASHBLK_SPELOCK_ADDR32,1U << (uint32)SectorId);
                /* Clear the lock bit successfully */
    ...
    else if((VIRTUAL_CODE_FLASH_BLOCK_0_FIRST_SEC_ID <= (uint32)Fls_VirtualSectors) && ((uint32)Fls_VirtualSectors <= VIRTUAL_CODE_FLASH_BLOCK_0_SSEC_REGION_LAST_SEC_ID))
    {
        ...
        */
        ...
        {
            u32TempLockMasterRegister = PFLASH_PFCBLK0_LOCKMASTER_SS0_ADDR32 + 0x0U + ((SectorId/4U)*4U);
            /*
            * @violates @ref ClearLock_c_REF_2 A conversion should not be performed between a pointer to object and an integer type
            * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
            */
            ...
            else
            {
                ...
                * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
                */
                REG_BIT_CLEAR32(PFLASH_PFCBLK0_SSPELOCK_ADDR32, (1U << SectorId));
                /* Clear the lock bit successfully */
                eReturnCode = FLS_JOB_OK;
            }
        }
        ...
        if(0U != REG_BIT_GET32(PFLASH_PFCBLK0_SETSLOCK_ADDR32, 1U << SectorId))
        {
            ...
            */
            ...
            else
            {
                ...
                * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
                */
                REG_BIT_CLEAR32(PFLASH_PFCBLK0_SPELOCK_ADDR32, (1U << SectorId));
                /* Clear the lock bit successfully */
                eReturnCode = FLS_JOB_OK;
            }
        }
        ...
        if(0U != REG_BIT_GET32(PFLASH_PFCBLK1_SSETSLOCK_ADDR32, (1U << SectorId)))
        {
            ...
            */
            ...
            else
            {
                ...
                * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
                */
                REG_BIT_CLEAR32(PFLASH_PFCBLK1_SSPELOCK_ADDR32,1U << SectorId);
                /* Clear the lock bit successfully */
                eReturnCode = FLS_JOB_OK;
            }
        }
        ...
        if(0U != REG_BIT_GET32(PFLASH_PFCBLK1_SETSLOCK_ADDR32, (1U << SectorId)))
        {
            ...
            */
            ...
            else
            {
                ...
                * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
                */
                REG_BIT_CLEAR32(PFLASH_PFCBLK1_SPELOCK_ADDR32, (1U << SectorId));
                /* Clear the lock bit successfully */
                eReturnCode = FLS_JOB_OK;
            }
        }
        ...
        if(0U != REG_BIT_GET32(PFLASH_PFCBLK2_SSETSLOCK_ADDR32, (1U << SectorId)))
        {
            ...
            */
            ...
            else
            {
                ...
                * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
                */
                REG_BIT_CLEAR32(PFLASH_PFCBLK2_SSPELOCK_ADDR32, (1U << SectorId));
                /* Clear the lock bit successfully */
                eReturnCode = FLS_JOB_OK;
            }
        }
        ...
        if(0U != REG_BIT_GET32(PFLASH_PFCBLK2_SETSLOCK_ADDR32, (1U << SectorId)))
        {
            ...
            */
            ...
            else
            {
                ...
                * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
                */
                REG_BIT_CLEAR32(PFLASH_PFCBLK2_SPELOCK_ADDR32, (1U << SectorId));
                /* Clear the lock bit successfully */
                eReturnCode = FLS_JOB_OK;
            }
        }
        ...
        if(0U != REG_BIT_GET32(PFLASH_PFCBLK3_SSETSLOCK_ADDR32, (1U << SectorId)))
        {
            ...
            */
            ...
            else
            {
                ...
                * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
                */
                REG_BIT_CLEAR32(PFLASH_PFCBLK3_SSPELOCK_ADDR32,(1U << SectorId));
                /* Clear the lock bit successfully */
                eReturnCode = FLS_JOB_OK;
            } 
        }
        ...
        if(0U != REG_BIT_GET32(PFLASH_PFCBLK3_SETSLOCK_ADDR32, (1U << SectorId)))
        {
            ...
            */
            ...
            else
            {
                ...
                * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
                */
                REG_BIT_CLEAR32(PFLASH_PFCBLK3_SPELOCK_ADDR32,(1U << SectorId));
                /* Clear the lock bit successfully */
                eReturnCode = FLS_JOB_OK;
            }
        }
        ...
        if(0U != REG_BIT_GET32(PFLASH_PFCBLKU_SETSLOCK_ADDR32, (1U << SectorId)))
        {
            ...
            */
            ...
            else
            {
                ...
                * @violates @ref ClearLock_c_REF_3 A cast shall not be performed between pointer to void and an arithmetic type
                */
                REG_BIT_CLEAR32(PFLASH_PFCBLKU_SPELOCK_ADDR32,PFLASH_PFCBLKU_SPELOCK_SLCK00_U32);
                /* Clear the lock bit successfully */
                eReturnCode = FLS_JOB_OK;
            }
        }
