...
do{                                                                           \
  register uint8_t domain_id = IP_XRDC->HWCFG1;                                  \
...
tFLASH_STATUS FLASH_ExecSeq (register uint32_t mask)//__tcm(func) tFLASH_STATUS FLASH_ExecSeq (register uint32_t mask)
{
  ...
  IP_FLASH->MCR |= mask;                          /* initiate sequence             */
  ...
  IP_FLASH->MCR &=~mask;                          /* close sequence                */
...
tFLASH_XSTATUS FLASH_XExecPgm (void)//__tcm(func) tFLASH_XSTATUS FLASH_XExecPgm (void)
{
  ...
  IP_FLASH->XMCR |= FLASH_XMCR_XPGM_MASK;         /* initiate express programming  */
  IP_FLASH->XMCR |= FLASH_XMCR_XEHV_MASK;         /* enable high voltage           */
  while(!(IP_FLASH->XMCR&FLASH_XMCR_XDONE_MASK)); /* wait until XMCR[XDONE]=1      */
  IP_FLASH->XMCR &=~FLASH_XMCR_XEHV_MASK;         /* disable high voltage          */
  xstatus = FLASH_GetXStatus();                /* read express interface status */
  IP_FLASH->XMCR &=~FLASH_XMCR_XPGM_MASK;         /* close express programming     */
...
{
  FLASH_InitSeq(dst);
  IP_FLASH->DATA[0] = 0UL;               /* one and only one DATA register written */
...
extern tFLASH_STATUS FLASH_Wr (const void *dst, void *src, int32_t nbytes)
{
  ...
  {
    pData = (uint32_t*)((uint32_t)&IP_FLASH->DATA[0] + ((uint32_t)pDst & FLASH_DATAX_MASK));
    FLASH_InitSeq (pDst);
    do {
      tmp = 0xffffffffUL;
...
tFLASH_XSTATUS FLASH_WrOnceCrashLog (const tCRASH_LOG dst, tCRASH_LOG src)
{
  ...
  {
    pData = (uint32_t*)((uint32_t)&IP_FLASH->DATA[0] + ((uint32_t)dst & FLASH_DATAX_MASK));
    FLASH_XInitSeq (dst);
    do {
