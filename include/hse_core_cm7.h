...
/* Always provide standard integer types regardless of whether the rest of this
 * file is processed.  This ensures int8_t / int32_t / uint32_t etc. are
 * available to any translation unit that only includes this header. */
#include <stdint.h>

/* Also skip this file when the NXP RTD/Platform SDK is present.
 * NXP's system.h (pulled in via Mcal.h -> OsIf_Internal.h) defines CCSIDR_WAYS
 * with NXP-style macro names.  When that is already defined we know the NXP
 * Platform SDK CMSIS has been included; re-processing this file would conflict
 * with its __NVIC_* / __STATIC_INLINE / IRQn_Type definitions. */
#if !defined(HSE_CORE_CM7_H_GENERIC) && !defined(__CORE_CM7_H_GENERIC) && \
    !defined(CCSIDR_WAYS)
#define HSE_CORE_CM7_H_GENERIC
...
#include "hse_cmsis_compiler.h"               /* CMSIS compiler specific defines */
...
#endif /* HSE_CORE_CM7_H_GENERIC */

/* Skip the device-function block when the NXP RTD Platform SDK is present;
 * its own CMSIS core already provides these definitions. */
#if !defined(__CMSIS_GENERIC) && !defined(CCSIDR_WAYS)

#if !defined(HSE_CORE_CM7_H_DEPENDANT) && !defined(__CORE_CM7_H_DEPENDANT)
#define HSE_CORE_CM7_H_DEPENDANT
...
#endif /* HSE_CORE_CM7_H_DEPENDANT */

#endif /* !__CMSIS_GENERIC && !CCSIDR_WAYS */
