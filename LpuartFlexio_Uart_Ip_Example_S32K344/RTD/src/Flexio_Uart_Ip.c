...
#define FLEXIO_UART_IP_SW_MAJOR_VERSION_C               6
...
static void Flexio_Uart_Ip_CheckTxOperation(const uint8 Channel, const uint16 ShifterMaskFlag, const uint16 TimerMaskFlag);
static void Flexio_Uart_Ip_ReadData(const uint8 Channel);
static void Flexio_Uart_Ip_CheckRxOperation(const uint8 Channel, const uint16 ShifterMaskFlag, const uint16 ShifterErrMaskFlag);
...
static void Flexio_Uart_Ip_StartReceiveDataUsingInt(const uint8 Channel);
...
void Flexio_Uart_Ip_IrqHandler(uint8 Channel, uint8 ShifterMaskFlag, uint8 ShifterErrorMaskFlag, uint8 TimerMaskFlag)
{
    ...
    Base = Flexio_Uart_Ip_apBases[UartUserCfg->Instance];
    ...
    {
        /* Clear timer status */
        Flexio_Mcl_Ip_ClearTimerStatus(Base, UartUserCfg->Channel);
        /* Clear Shifter status */
        Flexio_Mcl_Ip_ClearShifterStatus(Base, UartUserCfg->Channel);
        /*Clear Error Status flag*/
        Flexio_Mcl_Ip_ClearShifterErrorStatus(Base, UartUserCfg->Channel);
    }
    else
    {
        ...
        {
            Flexio_Uart_Ip_CheckTxOperation(Channel, (uint16)ShifterMaskFlag, (uint16)TimerMaskFlag);
        }
        else
        {
            Flexio_Uart_Ip_CheckRxOperation(Channel, (uint16)ShifterMaskFlag, (uint16)ShifterErrorMaskFlag);
        }
    }
}
                                      ...
                                      )
{
    ...
    Flexio_Uart_Ip_SetTimerCompare(Base, TIMER(UartUserCfg->Channel), (uint16)((((uint16)(Bits << 1U) - 1U) << 8U) + ((uint8)(Divider/2U) - 1U)));
...
static void Flexio_Uart_Ip_EndTransfer(const uint8 Channel)
{
    ...
    Flexio_Mcl_Ip_SetTimerInterrupt(Base, (uint8)(1U << TIMER(UartUserCfg->Channel)), FALSE);
...
static void Flexio_Uart_Ip_StopTransfer(const uint8 Channel)
{
    ...
    Flexio_Uart_Ip_SetTimerMode(Base, TIMER(UartUserCfg->Channel), FLEXIO_TIMER_MODE_DISABLED);
    Flexio_Uart_Ip_SetShifterMode(Base, SHIFTER(UartUserCfg->Channel), FLEXIO_SHIFTER_MODE_DISABLED);

    /* Clear any leftover error flags */
    Flexio_Mcl_Ip_ClearShifterErrorStatus(Base, SHIFTER(UartUserCfg->Channel));
    ...
    if (FLEXIO_UART_IP_DIRECTION_TX == UartUserCfg->Direction)
    {
        ...
        Flexio_Uart_Ip_SetShifterStartBit(Base, SHIFTER(UartUserCfg->Channel), FLEXIO_SHIFTER_START_BIT_0);
        Flexio_Uart_Ip_SetShifterMode(Base, SHIFTER(UartUserCfg->Channel), FLEXIO_SHIFTER_MODE_TRANSMIT);
    }
}
...
static void Flexio_Uart_Ip_WriteData(const uint8 Channel)
{
    ...
    Flexio_Uart_Ip_WriteShifterBuffer(Base, SHIFTER(UartUserCfg->Channel), Data32b);
}
...
static void Flexio_Uart_Ip_CheckTxOperation(const uint8 Channel, const uint16 ShifterMaskFlag, const uint16 TimerMaskFlag)
{
    ...
    UartState = (Flexio_Uart_Ip_StateStructureType *)Flexio_Uart_Ip_apStateStructuresArray[Channel];
    UartUserCfg = Flexio_Uart_Ip_apUserConfig[Channel];
    ResourceMask = (1UL << UartUserCfg->Channel);
    Base = Flexio_Uart_Ip_apBases[UartUserCfg->Instance];
    ...
    if (0U == UartState->RemainingBytes)
    {
        ...
        {
            if (!Flexio_Uart_Ip_GetTimerInterrupt(Base,TIMER(UartUserCfg->Channel)))
            {
    ...
    {
        if (!Flexio_Uart_Ip_GetShifterInterrupt(Base, SHIFTER(UartUserCfg->Channel)))
        {
            /* CPR_RTD_00664.uart Spurious interrupt*/
            /* Do nothing - Return immediately */
        }
...
static void Flexio_Uart_Ip_CheckCompleteTransferData(const uint8 Channel)
{
    ...
    if (0U == UartState->RemainingBytes)
    {
        ...
        Flexio_Mcl_Ip_ClearTimerStatus(Base, TIMER(UartUserCfg->Channel));
        if (FLEXIO_UART_IP_DRIVER_TYPE_INTERRUPTS == UartUserCfg->DriverType)
        {
            /* Transmission completed; disable interrupt */
            Flexio_Mcl_Ip_SetShifterInterrupt(Base, (uint8)(1U << SHIFTER(UartUserCfg->Channel)), FALSE);
            /* Enable timer interrupt to ensure that transfer is completed */
            Flexio_Mcl_Ip_SetTimerInterrupt(Base, (uint8)(1U << TIMER(UartUserCfg->Channel)), TRUE);
        }
    }
}
...
static void Flexio_Uart_Ip_ReadData(const uint8 Channel)
{
    ...
    Data32b = Flexio_Uart_Ip_ReadShifterBuffer(Base, SHIFTER(UartUserCfg->Channel));
...
static void Flexio_Uart_Ip_CheckRxOperation(const uint8 Channel, const uint16 ShifterMaskFlag, const uint16 ShifterErrMaskFlag)
{
    ...
    {
        if (!Flexio_Uart_Ip_GetShifterErrorInterrupt(Base, SHIFTER(UartUserCfg->Channel)))
        {
            /* CPR_RTD_00664.uart Spurious interrupt*/
            /* Do nothing - Return immediately */
            IsReturn = TRUE;
    ...
    if ((0U == UartState->RemainingBytes) && (!IsReturn))
    {
        ...
        if (FLEXIO_UART_IP_DRIVER_TYPE_DMA == UartUserCfg->DriverType)
        {
            ...
            Flexio_Mcl_Ip_SetShifterDMARequest(Base, (uint8)(1U << SHIFTER(UartUserCfg->Channel)), FALSE);
        }
#endif
        /* Discard any leftover rx data */
        Flexio_Mcl_Ip_ClearShifterStatus(Base, SHIFTER(UartUserCfg->Channel));
        /* End transfer */
...
static void Flexio_Uart_Ip_CheckShifterErrorStatus(const uint8 Channel)
{
    ...
    if (Flexio_Mcl_Ip_GetShifterErrorStatus(Base, SHIFTER(UartUserCfg->Channel)))
    {
        UartState->Status = FLEXIO_UART_IP_STATUS_RX_OVERRUN;
        Flexio_Mcl_Ip_ClearShifterErrorStatus(Base, SHIFTER(UartUserCfg->Channel));
    }
...
static void Flexio_Uart_Ip_SetShifterInterrupt(const uint8 Channel, boolean Enable)
{
    ...
    Flexio_Mcl_Ip_SetShifterInterrupt(Base, (uint8)(1U << UartUserCfg->Channel), Enable);
    Flexio_Mcl_Ip_SetShifterErrorInterrupt(Base, (uint8)(1U << UartUserCfg->Channel), Enable);
}
...
static void Flexio_Uart_Ip_EnableReceiver(const uint8 Channel)
{
    ...
    Flexio_Mcl_Ip_ClearShifterStatus(Base, SHIFTER(UartUserCfg->Channel));
    Flexio_Uart_Ip_SetShifterMode(Base, SHIFTER(UartUserCfg->Channel), FLEXIO_SHIFTER_MODE_RECEIVE);
    Flexio_Uart_Ip_SetTimerMode(Base, TIMER(UartUserCfg->Channel), FLEXIO_TIMER_MODE_8BIT_BAUD);
}
...
static Flexio_Uart_Ip_StatusType Flexio_Uart_Ip_StartSendDataUsingDma(uint8 Channel, const uint8 * TxData, uint32 TxSize)
{
    ...
    Base = Flexio_Uart_Ip_apBases[UserConfig->Instance];
    ...
    DmaTransferList[1].Value = FLEXIO_UART_IP_LSBW_ADDR(Base->SHIFTBUF[UserConfig->Channel]);
    ...
    Flexio_Mcl_Ip_SetShifterDMARequest(Base, (uint8)(1U << SHIFTER(UserConfig->Channel)), TRUE);
...
static Flexio_Uart_Ip_StatusType Flexio_Uart_Ip_StartReceiveDataUsingDma(uint8 Channel, uint8 * RxBuff, uint32 RxSize)
{
    ...
    /* Set up parameters for Dma_Ip_LogicChannelTransferListType */
    DmaTransferList[0].Param = DMA_IP_CH_SET_SOURCE_ADDRESS;
    DmaTransferList[0].Value = FLEXIO_UART_IP_LSBW_ADDR(Base->SHIFTBUF[UserConfig->Channel]) + 3U;
    DmaTransferList[1].Param = DMA_IP_CH_SET_DESTINATION_ADDRESS;
...
void Flexio_Uart_Ip_Init(const uint8 Channel, const Flexio_Uart_Ip_UserConfigType * UserConfig)
{
    ...
    FLEXIO_UART_IP_DEV_ASSERT(UserConfig->Instance < FLEXIO_INSTANCE_COUNT);
    if (1U == UserConfig->Instance)
    {
        FLEXIO_UART_IP_DEV_ASSERT(Channel == (UserConfig->Channel + 16U));
    }
    else
    {
...
Flexio_Uart_Ip_StatusType Flexio_Uart_Ip_Deinit(const uint8 Channel)
{
    ...
    /* Check if driver is busy, return ERROR report */
    ...
    else
    {
        ...
        switch (UartUserCfg->DriverType)
        {
            case FLEXIO_UART_IP_DRIVER_TYPE_INTERRUPTS:
                /* Clear Error Status flag before enabling  error interrupt*/
                Flexio_Mcl_Ip_ClearShifterErrorStatus(Base, Channel);
                /* Don't clear the shiter flag due to it always set  when initially configured for Transmit , 
                it will be used to jump into the interrupt, when enabling interrupt */
...
Flexio_Uart_Ip_StatusType Flexio_Uart_Ip_AsyncReceive(const uint8 Channel, uint8 * RxBuff, const uint32 RxSize)
{
    ...
    else
    {
        ...
        {
            case FLEXIO_UART_IP_DRIVER_TYPE_INTERRUPTS:
            Flexio_Uart_Ip_StartReceiveDataUsingInt(Channel);
            RetStatus = FLEXIO_UART_IP_STATUS_SUCCESS;
            break;
#if (FLEXIO_UART_IP_HAS_DMA_ENABLED == STD_ON)
            case FLEXIO_UART_IP_DRIVER_TYPE_DMA:
                /* Clear Error Status flag */
                Flexio_Mcl_Ip_ClearShifterErrorStatus(Base, Channel);
                /* Enable interrupt errors to verify when the error happens */
                Flexio_Mcl_Ip_SetShifterErrorInterrupt(Base, (uint8)(1U << UartUserCfg->Channel), TRUE);
                                                    ...
                                                    )
{
    ...
    else
    {
        ...
        Flexio_Uart_Ip_SetTimerCompare(Base, TIMER(UartUserCfg->Channel), (uint16)((((uint16)(BitCountValue << 1U) - 1U) << 8U) + (uint8)CMP_value));

        if (FLEXIO_UART_IP_DIRECTION_TX == UartUserCfg->Direction)
        {
            /* Configure tx timer */
            Flexio_Uart_Ip_SetTimerConfig(Base,
                                          TIMER(UartUserCfg->Channel),
...
static void Flexio_Uart_Ip_CompleteTxOperation(const uint8 Channel)
{
    ...
    else if (Flexio_Mcl_Ip_GetShifterStatus(Base, SHIFTER(UartUserCfg->Channel)))
    {
        ...
        Flexio_Uart_Ip_SetShifterStartBit(Base, SHIFTER(UartUserCfg->Channel), FLEXIO_SHIFTER_START_BIT_1);
        /* Writes data in the specified shifter buffer.*/
        Flexio_Uart_Ip_WriteShifterBuffer(Base, SHIFTER(UartUserCfg->Channel), 0xFFFFFFFFUL);
    }
    else
    {
...
static void Flexio_Uart_Ip_SyncSendData(const uint8 Channel, const uint32 Timeout)
{
    ...
    {
        Flexio_Uart_Ip_WriteData(Channel);
        while (!Flexio_Mcl_Ip_GetShifterStatus(Base, UartUserCfg->Channel)  && \
               !Flexio_Uart_Ip_CheckTimeout(&StartTime, &ElapsedTicks, TimeoutTicks, FLEXIO_UART_IP_TIMEOUT_TYPE) \
              )
        {}
    }
...
static void Flexio_Uart_Ip_CompleteRxOperationUsingDma(const uint8 Channel)
{
    ...
    if (FLEXIO_UART_IP_STATUS_ABORTED == UartState->Status)
    {
        (void)Flexio_Uart_Ip_ReadShifterBuffer(Base, SHIFTER(UartUserCfg->Channel));
    }
    /* End transfer */
    Flexio_Uart_Ip_StopTransfer(Channel);
...
static void Flexio_Uart_Ip_ConfigureRxTimer(const uint8 Channel, const uint8 DataPin, const Flexio_Uart_Ip_TimerDecrementType TimerDec)
{
    ...
    Flexio_Uart_Ip_SetTimerControl(Base, TIMER(UartUserCfg->Channel), FLEXIO_TIMER_MODE_DISABLED);
...
static void Flexio_Uart_Ip_ConfigureTxTimer(const uint8 Channel, const Flexio_Uart_Ip_TimerDecrementType TimerDec)
{
    ...
    Flexio_Uart_Ip_SetTimerTrigger(Base,
                                   TIMER(UartUserCfg->Channel),
                                   (uint8)((uint8)(SHIFTER(UartUserCfg->Channel) << 2U) + 1U), /* Trigger on tx shifter status flag */
...
static void Flexio_Uart_Ip_CheckTransferCompleteData(const uint8 Channel)
{
    ...
    if (FLEXIO_UART_IP_DIRECTION_TX == UartUserCfg->Direction)
    {
        while (!Flexio_Mcl_Ip_GetTimerStatus(Base, TIMER(UartUserCfg->Channel)) && \
               !Flexio_Uart_Ip_CheckTimeout(&StartTime, &ElapsedTicks, TimeoutTicks, FLEXIO_UART_IP_TIMEOUT_TYPE) \
              )
        {}
    }
    ...
    {
        while (!Flexio_Mcl_Ip_GetShifterStatus(Base, SHIFTER(UartUserCfg->Channel)) && \
               !Flexio_Uart_Ip_CheckTimeout(&StartTime, &ElapsedTicks, TimeoutTicks, FLEXIO_UART_IP_TIMEOUT_TYPE) \
              )
        {}
        if (Flexio_Mcl_Ip_GetShifterStatus(Base, SHIFTER(UartUserCfg->Channel)))
        {
            /* Read Dummy */
            (void)Flexio_Uart_Ip_ReadShifterBuffer(Base, SHIFTER(UartUserCfg->Channel));
        }
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Flexio_Uart_Ip_StartReceiveDataUsingInt
 * Description   : This function will be waited until the data is completely transfer the data.
 *
 *END**************************************************************************/
static void Flexio_Uart_Ip_StartReceiveDataUsingInt(const uint8 Channel)
{
    ...
    /* Clear shifter and error status flag before enabling interrupts */
    /* Clear Shifter status */
