...
#define FLEXIO_MCL_IP_SW_MAJOR_VERSION_C               6
...
Flexio_Ip_CommonStatusType Flexio_Mcl_Ip_InitDevice(const Flexio_Ip_InstanceInitType * const pFlexioInitType)
{
    uint8 LocCfgIdx;
    uint8 LocInstance;
    uint32 NumLogicInstance = pFlexioInitType->NumOfInstanceConfig;
    boolean LocDebugEnable;
    FLEXIO_Type *BaseAddr;
    Flexio_Ip_CommonStatusType Status = FLEXIO_IP_COMMON_STATUS_SUCCESS;

    if (0U < NumLogicInstance)
    {
        for (LocCfgIdx = 0U; LocCfgIdx < NumLogicInstance; LocCfgIdx++)
        {
            LocInstance =  (*(pFlexioInitType->pInstanceConfig))[LocCfgIdx].instance;
            LocDebugEnable = (*(pFlexioInitType->pInstanceConfig))[LocCfgIdx].debugEnable;
...
*/
void Flexio_Mcl_Ip_DeinitDevice(const Flexio_Ip_InstanceInitType * const pFlexioInitType)
{
    FLEXIO_Type *BaseAddr;
    uint8 LocCfgIdx;
    uint32 NumLogicInstance = pFlexioInitType->NumOfInstanceConfig;
    ...
    if (0U < NumLogicInstance)
    {
        for (LocCfgIdx = 0U; LocCfgIdx < NumLogicInstance; LocCfgIdx++)
        {
            BaseAddr = Flexio_Ip_paxBase[(*(pFlexioInitType->pInstanceConfig))[LocCfgIdx].instance];
            /* Reset FlexIO module */
            Flexio_Mcl_Ip_Init(BaseAddr);
            Flexio_Ip_baIpIsInitialized[(*(pFlexioInitType->pInstanceConfig))[LocCfgIdx].instance] = FALSE;
